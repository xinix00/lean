...
| **Inkomende broadcast-IP** | Uitgaand kán het sinds 12-08 (255.255.255.255 en het subnet-broadcastadres gaan naar `ff:ff:ff:ff:ff:ff`, nooit via ARP) omdat een DHCP-rebind het nodig heeft. Inkomend blijft dicht: de UDP-checksum wordt over het bestemmings-IP berekend, dus een broadcast aannemen vraagt dat de echte `dst` door de hele demux meereist — en de enige klant (de rebind-ACK) komt unicast op `ciaddr` terug omdat wij de broadcast-flag uit laten. | Als een protocol een broadcast-antwoord eist dat niet naar ons eigen adres komt (mDNS, NetBIOS). Dan is het `dst` door `recvIPv4` en `ChecksumOK` heen dragen, plus een filter dat alleen de gebonden poorten wekt. |
