...
// De key schedule tegen de UITGEWERKTE handshake van RFC 8448 §3 ("Simple
// 1-RTT Handshake"). Dat document bestaat precies hiervoor: het geeft elke
// tussenwaarde van een echte verbinding in hex, dus een fout in een label, in
// de argumentvolgorde van HKDF-Extract of in een transcript-hash valt hier om
// in plaats van pas bij een Finished die niet klopt.
//
// Alle waarden hieronder zijn letterlijk uit die RFC geknipt.
...
func TestScheduleRFC8448(t *testing.T) {
	...
	// Stap voor stap, zodat een fout aanwijst wélke stap.
	...
	// En de sleutels die de server er daadwerkelijk mee schreef.
	k := keysFrom(shs)
...
// De nonce-constructie van §5.3: het recordnummer rechts uitgelijnd in een
// veld van IV-lengte, dan XOR met de IV. Record 0 geeft dus de IV zelf terug.
func TestNonce(t *testing.T) {
	...
	// Record 1 verschilt alleen in de laatste byte (0x30 ^ 1 = 0x31).
	got := nonce(iv, 1)
	if !bytes.Equal(got[:11], iv[:11]) || got[11] != iv[11]^1 {
		t.Errorf("record 1: %x", got)
	}
	// Een recordnummer dat 32 bits overschrijdt hoort de bovenste helft óók te
	// raken — dat is precies waar een 32-bits teller stil fout zou gaan.
	got = nonce(iv, 1<<40)
